...
readonly SATSOLVER="$TOOLBASEDIR/../kissat"
SATSOLVER_PARAMS=""
...
readonly certificate="$TMPDIR/qbfcert_certificate$$.aig"
...
QBFSOLVER_PARAMS+=" --trace=$qrp_format  --traditional-qcdcl"
...
/usr/bin/time -q -f "solving-time: %e" $QBFSOLVER $QBFSOLVER_PARAMS < "$formula" > "$trace" #2>> "$log_file"
solver_result=$?

trace_size=`ls -la $trace | cut -f5 -d" "`
echo "trace-size: $trace_size"

echo "solving-res $solver_result"
...
if [[ $solver_result == $QBFSOLVER_UNSAT ]]; then
  ...
  /usr/bin/time -q -f "qrpcheck-time: %e" $QRPCHECK $QRPCHECK_PARAMS "$trace" > "$proof" 
  qrpcheck_result=$?

  proof_size=`ls -la $proof | cut -f5 -d" "`
...
/usr/bin/time -q -f "qrpcert-time: %e" $QRPCERT $QRPCERT_PARAMS "$proof" > "$certificate"
qrpcert_result=$?

cert_size=`ls -la $certificate | cut -f5 -d" "`
cert_header=`grep aag $certificate`

echo "cert-size: $cert_size"
echo "cert-header: $cert_header"
...
log ""
/usr/bin/time -q -f "certcheck-time: %e" $CERTCHECK $CERTCHECK_PARAMS "$formula" "$certificate" > "$merged_cnf"
certcheck_result=$?

merged_header=`grep "p cnf" $merged_cnf`
merged_size=`ls -la $merged_cnf | cut -f5 -d" "`

echo "merged-header: $merged_header"
echo "merged-size: $merged_size"
...
log ""
/usr/bin/time -q -f "final-check-time: %e" $SATSOLVER $SATSOLVER_PARAMS "$merged_cnf" 
sat_solver_result=$?
